...
interface EstimateFormProps {
  ...
  initialData?: (CreateEstimateData & { materials: Material[] }) | null;
}
...
const EstimateForm: React.FC<EstimateFormProps> = ({ onSubmit, initialData }) => {
  ...
  const [materials, setMaterials] = useState<Omit<Material, 'id' | 'estimateId'>[]>([]);
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    onSubmit({ title, description, clientId, laborCost, materialsTotal, totalCost, materials });
  };
  ...
  return (
    <Box component="form" onSubmit={handleSubmit} sx={{ mt: 1 }}>
      <Grid container spacing={2}>
        ...
        <Grid item xs={12} sm={6}>
          <TextField label="Labor Cost" type="number" value={laborCost} onChange={(e) => setLaborCost(parseFloat(e.target.value))} fullWidth />
        </Grid>
      </Grid>
      ...
      {materials.map((material, index) => (
        <Grid container spacing={1} key={index} alignItems="center" sx={{ mb: 2 }}>
          <Grid item xs={4}><TextField label="Name" value={material.name} onChange={(e) => handleMaterialChange(index, 'name', e.target.value)} fullWidth /></Grid>
          <Grid item xs={2}><TextField label="Quantity" type="number" value={material.quantity} onChange={(e) => handleMaterialChange(index, 'quantity', parseFloat(e.target.value))} fullWidth /></Grid>
          <Grid item xs={2}><TextField label="Unit Price" type="number" value={material.unitPrice} onChange={(e) => handleMaterialChange(index, 'unitPrice', parseFloat(e.target.value))} fullWidth /></Grid>
          <Grid item xs={2}><Typography>Total: ${(material.quantity * material.unitPrice).toFixed(2)}</Typography></Grid>
          <Grid item xs={2}><IconButton onClick={() => removeMaterial(index)} color="error"><DeleteIcon /></IconButton></Grid>
        </Grid>
      ))}
      <Button startIcon={<AddCircleOutlineIcon />} onClick={addMaterial} sx={{ mt: 1 }}>
        Add Material
      </Button>
